...
{
    // 配置 SPI 时钟
    SPIConfigClkFormat(SOC_SPI_1_REGS, (SPI_CLK_POL_LOW | SPI_CLK_OUTOFPHASE), dataFormat);
...
void SPIWrite(unsigned char cs, unsigned char *data, unsigned int len)
{
    while(len)
    {
        ...
        len--;
        SPITransmitData1(SOC_SPI_1_REGS, data[len]);
...
void SPIInit(unsigned char cs)
{
    ...
    // SPI 时钟
    SPIClkConfigure(SOC_SPI_1_REGS, 228000000, 50000000, SPI_DATA_FORMAT0);
